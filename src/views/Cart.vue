  ...
  <div style="width:100%;height: 100vh;">
    ...
    <template v-if="isTheCartHaving">
      <van-row class="cart-list">
        <van-cell-group inset class="cart-item" v-for="(shop, i) in myCart" :key="i">
          <van-cell class="item-head" style="background-color: #FAFAFA;box-sizing: content-box;">
            <van-checkbox-group v-model="itemTotal">
              <van-checkbox :name="(shop.shop.id - 1).toString()" label-disabled>
                <van-row>
                  <van-col>
          ...
          <van-cell class="item-body">
            <van-checkbox-group v-model="itemSingle[shop.shop.id - 1]">
              <template v-for="(food, j) in shop.foods" :key="j">
                <van-checkbox :name="j.toString()" v-if="food.count > 0" label-disabled>
                  <van-row style="position: relative;width: 75vw;" type="flex" align="center">
                    ...
                    <van-col style="position: absolute;top:40px;left: 170px;">
                      <van-stepper v-model="$store.state.the_cart[shop.shop.id - 1].foods[j].count" min="0"
                        @click.stop />
                    </van-col>
                  </van-row>
                </van-checkbox>
      ...
      <van-row type="flex" justify="space-between" align="center" class="cart-footer">
        <van-col span="4">
          <van-checkbox style="font-size: 12px;width: 90%;align-items: flex-start;" v-model="checked">全选
          </van-checkbox>
        </van-col>
        ...
        <van-col span="6">
          <van-button round :color="!isTheCartHaving ? 'lightgray' : 'dodgerblue'" :disabled="!isTheCartHaving"
            style="float: right;height: 36px;width: 30vw;" @click="sendDB">一键结算
          </van-button>
        </van-col>
      </van-row>
    </template>
...
export default {
  ...
  data() {
    return {
      ...
      backColor: '#f5f5f5',
      backImage: new URL('../assets/images/ic_back.png', import.meta.url).href,
      lilBackImage: new URL('../assets/images/ap_arrow_right_gray.png', import.meta.url).href,
      trashImage: new URL('../assets/images/ap_search_buttom_delete.png', import.meta.url).href,
